...
@Test func sceneJSON_encodes_deterministically() async throws {
    ...
    if #available(macOS 10.13, *) { encoder.outputFormatting = [.sortedKeys] }
...
@Test func sceneJSON_matches_golden_fixture() async throws {
    // Build the same scene as in the minimal test
    let page = Components.Schemas.PageSpec(
        widthPt: 200,
        heightPt: 100,
        margins: .init(top: 10, left: 10, right: 10, bottom: 10),
    ...
    // Load golden fixture, decode to Scene, re-encode canonically, and compare bytes
    let url = URL(fileURLWithPath: #filePath).deletingLastPathComponent().appendingPathComponent("Fixtures/scene_minimal.json")
    let goldenData = try Data(contentsOf: url)
    let decoder = JSONDecoder()
    let goldenScene = try decoder.decode(Components.Schemas.Scene.self, from: goldenData)
    let goldenCanonical = try encoder.encode(goldenScene)
    #expect(data == goldenCanonical)
}
